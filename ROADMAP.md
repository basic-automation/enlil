...
- [ ] 3.10 qcow2 refcount-table growth (`storage/qcow.rs`) — needs interior-mutable `QcowHeader`; image-corruption risk, do carefully. **Slice landed:** `QcowHeader::refcount_table_clusters_for(rt_index)` computes the grown table size (pure arithmetic, tested), and `allocate_refcount_block`'s overflow error now reports exactly how many table clusters are needed. **Next slice:** make the refcount-table geometry (`refcount_table_offset`/`_clusters`) interior-mutable, then implement the on-disk grow — allocate the larger table at EOF, copy entries, refcount the new table clusters (recursively via the now-larger table), rewrite the header last, free the old table — gated on `check_consistency` passing after every grow
...
- [x] 3.13 Optimize `fire_due_tsc_deadlines`: skips the `KVM_GET_MSRS` guest-TSC read when no LAPIC has an armed deadline — `StandardPc::any_lapic_tsc_deadline_armed` (a non-zero `IA32_TSC_DEADLINE` only ever exists in TSC-deadline mode) gates it. Test `any_lapic_tsc_deadline_armed_tracks_the_armed_state`
- [x] 3.14 RTC/CMOS wall-clock cadence — `run_real_mode` advances the MC146818 calendar by real host wall-clock seconds each entry (`StandardPc::advance_rtc_seconds`, capped at `RTC_MAX_CATCHUP_SECONDS`) so a long-running guest's real-time clock does not freeze (a correctness gap + a mild tell); wall-driven, not guest-cycle driven, so a sub-second run ticks nothing. Test `advance_rtc_seconds_ticks_the_calendar_and_fires_updates`
- [ ] 3.15 Drive the RTC **periodic** interrupt (`Rtc146818::tick_periodic` at `periodic_rate_hz`) from the run loop — some guests use it as a tick source; still has no production caller
