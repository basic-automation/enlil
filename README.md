...
| Bare-metal UEFI boot (Phase 6) | ⏳ Stub (`enlil-boot`) — firmware-table discovery pipeline landed: `discover::build_device_tree` assembles a `HostDeviceTree` in one call from the ACPI tables — CPUs with NUMA nodes (MADT + SRAT type-0/x2APIC affinity), per-node RAM ranges and node-to-node distances (SRAT/SLIT), a full MCFG-ECAM PCI config-space walk (functions, BARs, xHCI MMIO bases, capability lists incl. SR-IOV/MSI-X, subsystem IDs), and DMAR/IVRS IOMMU kind; plus `MemoryMap` carve/plan from E820 & UEFI maps and reusable x86-64 identity page-table builders |
