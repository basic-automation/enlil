...
| **`enlil-boot`** | UEFI boot payload + early kernel for the bare-metal target (Phase 6). A real `efi_main` collects the boot handoff and calls `ExitBootServices()`, then `kernel_entry` brings the machine up under enlil's own control: memory-map walk, switching heap, IDT + `int3` self-test, x2APIC, AMD SVM enable + `VM_HSAVE_PA`, **running a trivial guest to `#VMEXIT(HLT)` with `VMRUN`** (VMCB + NPT built via `enlil-hal`), and GOP framebuffer — QEMU+OVMF boot-proven nightly. |
