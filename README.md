...
| Bare-metal UEFI boot (Phase 6) | 🚧 Foundations landing (`enlil-boot`, `enlil-hal`, `enlil-platform`) — the UEFI payload has a real `efi_main` that collects the ACPI RSDP + GOP framebuffer + final UEFI memory map into a `BootHandoff`, calls `ExitBootServices()`, and prints its liveness banner straight to COM1 serial afterward (build-verified for `x86_64-unknown-uefi`; QEMU+OVMF boot proof pending a local qemu/ovmf install). The bare-metal `GlobalAlloc` is now backed by a real heap installed from the firmware memory map. The Intel VMX seam in `enlil-hal::vmx` decodes the VMX capability MSRs (`IA32_VMX_BASIC`, the pin/proc/exit/entry control MSRs), the VMCS field encodings and VM-exit reason/qualification fields, routes I/O·HLT·triple-fault exits onto the arch-neutral `VmExit`, and stamps VMXON/VMCS regions — all built for the `x86_64-unknown-enlil` kernel target. Still landed: `discover::build_device_tree` assembles a `HostDeviceTree` from the ACPI tables (CPUs with NUMA nodes via MADT + SRAT, per-node RAM ranges and distances via SRAT/SLIT, a full MCFG-ECAM PCI walk incl. BARs/xHCI/capabilities/SR-IOV, DMAR/IVRS IOMMU kind), plus `MemoryMap` carve/plan from E820 & UEFI maps and x86-64 identity page-table builders |
