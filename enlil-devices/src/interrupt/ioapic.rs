...
impl IoApic {
    pub fn new(id: u8) -> Self {
        Self {
            id,
            reg_select: 0,
            entries: [RedirectionEntry::default(); NUM_IOAPIC_PINS],
    ...
    pub fn mmio_read(&self, offset: u64) -> u32 {
        match offset {
            IOREGSEL => self.reg_select as u32,
    ...
    fn read_register(&self, reg: u8) -> u32 {
        match reg {
            ...
            IOAPIC_REG_VER => {
                IOAPIC_VERSION | (((NUM_IOAPIC_PINS - 1) as u32) << 16)
            }
            IOAPIC_REG_ARB => (self.id as u32) << 24,
            r if r >= IOAPIC_REG_REDTBL_BASE => {
                let index = (r - IOAPIC_REG_REDTBL_BASE) as usize;
    ...
    pub fn set_irq(&mut self, irq: usize) -> Option<InterruptRoute> {
    ...
    pub fn get_rte(&self, irq: u8) -> RedirectionEntry {
        self.entries[irq as usize]
    }

    /// Get a mutable reference to a redirection table entry by IRQ number.
    pub fn get_rte_mut(&mut self, irq: u8) -> &mut RedirectionEntry {
        &mut self.entries[irq as usize]
    }

    /// Get a reference to a redirection entry.
    pub fn entry(&self, irq: usize) -> Option<&RedirectionEntry> {
        self.entries.get(irq)
    }
}
...
mod tests {
    ...
    fn ioapic_read_version() {
        ...
        ioapic.mmio_write(IOREGSEL, IOAPIC_REG_VER as u32);
